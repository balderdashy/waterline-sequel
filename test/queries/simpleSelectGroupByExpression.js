module.exports = {
  ...
  expected   : {
    ...
    select: {
      ...
      queryString    : 'SELECT `foo`.`bar`::integer as group0, MAX(`foo`.`baz`) AS baz FROM `foo` AS `foo` ',
    ...
    find  : {
      ...
      // The queryString we expect to be rendered after calling `Sequel.select()`
      queryString    : 'SELECT `foo`.`bar`::integer as group0, MAX(`foo`.`baz`) AS baz FROM `foo` AS `foo`   GROUP BY `foo`.`bar`::integer',
