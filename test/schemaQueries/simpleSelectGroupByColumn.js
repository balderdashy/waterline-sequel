module.exports = {
  ...
  expected   : {
    ...
    select: {
      ...
      queryString    : 'SELECT `foo`.`bar`, MAX(`foo`.`baz`) AS baz FROM `myschema`.`foo` AS `foo` ',
    ...
    find  : {
      ...
      // The queryString we expect to be rendered after calling `Sequel.select()`
      queryString    : 'SELECT `foo`.`bar`, MAX(`foo`.`baz`) AS baz FROM `myschema`.`foo` AS `foo`   GROUP BY `foo`.`bar`',
